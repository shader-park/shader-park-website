...
export function createPedestalEdges(pedSize, pedHeight, size = 0.005) {
  ...
  const longEdgeGeom =
      new THREE.BoxBufferGeometry(edgeSize, edgeSize, pedSize + 1.0 * edgeSize);
  const shortEdgeGeomVertical =
      new THREE.BoxBufferGeometry(edgeSize, pedHeight, edgeSize);
  const shortEdgeGeomHorizontal =
      new THREE.BoxBufferGeometry(pedSize, edgeSize, edgeSize);
  const edgeMat =
      new THREE.MeshBasicMaterial({color: new THREE.Color(1.0, 1.0, 1.0)});
